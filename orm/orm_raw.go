...
func (o *rawSet) QueryRow(colName string,containers ...interface{}) error {
	var (
...
func (o *rawSet) QueryRows(colName string,containers ...interface{}) (int64, error) {
	var (
		refs  = make([]interface{}, 0, len(containers))
		sInds []reflect.Value
		eTyps []reflect.Type
			...
			if sMi != nil {
				...
				recursiveSetField = func(rv reflect.Value) {
					for i := 0; i < rv.NumField(); i++ {
						...
						//d1,_:=json.Marshal(fe.Tag.Get(defaultStructTagName))
						//println(string(d1))
						//d1,_:=json.Marshal(tags)
						//println(string(d1))
						if v, ok := columnsMp[col]; ok {
							value := reflect.ValueOf(v).Elem().Interface()
							o.setFieldValue(f, value)
